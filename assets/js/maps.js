...
// Creates a new HTML input for text and date time using jquery then assigns the elements to a location and datetime variable for google autocomplete and generic value storage.
  initalise(){
    $(`<input type="text" class="col-7 form-control" id="${this.locationData.id}-input" name="${this.locationData.id}-input" placeholder="Search Destination">
       <input type="datetime-local" class="col-5 form-control" id="${this.locationData.id}-date" name="${this.locationData.id}-date">`)
       .appendTo(".route-form");
  ...
  setUpdateAutocomplete(){
    const location = new google.maps.places.Autocomplete(document.getElementById(`${this.locationData.id}-input`));
...
class WayPointsData{
  constructor(){
    ...
    this.origin = () => {
      const startPoint = this.locations.find(location =>{ return location.id === "origin"});
      return startPoint.location.formatted_address;
    };
    this.destination = () => {
      const endPoint = this.locations.find(location =>{ return location.id === "destination"});
